...
const Writer = struct {
    ...
    fn writeInstToStream(
        ...
        switch (tag) {
            ...
            .@"switch",
            .switch_ref,
            => try self.writeSwitch(stream, inst),
    ...
    fn writeSwitchBlockErrUnion(self: *Writer, stream: *std.Io.Writer, inst: Zir.Inst.Index) !void {
        ...
        {
            const info = @as(Zir.Inst.Switch.ProngInfo, @bitCast(self.code.extra[extra_index]));
            extra_index += 1;
    ...
    fn writeSwitch(self: *Writer, stream: *std.Io.Writer, inst: Zir.Inst.Index) !void {
        const zir_switch = self.code.getSwitch(inst);
        var extra_index = zir_switch.end;

        try self.writeInstRef(stream, zir_switch.operand);
        if (zir_switch.tag_capture_inst.unwrap()) |tag_capture_inst| {
        ...
        while (case_it.next()) |case| {
            try stream.writeAll(",\n");
            try stream.splatByteAll(' ', self.indent);
            switch (case.prong_info.capture) {
                .none => {},
                .by_val => try stream.writeAll("by_val "),
                .by_ref => try stream.writeAll("by_ref "),
            }
            if (case.prong_info.is_inline) try stream.writeAll("inline ");
            ...
            for (case.item_body_lens) |item_body_len| {
                const item_body = self.code.bodySlice(extra_index, item_body_len);
            ...
            for (case.range_bodies_lens) |range_bodies_len| {
                const first_body = self.code.bodySlice(extra_index, range_bodies_len[0]);
                extra_index += first_body.len;
                const last_body = self.code.bodySlice(extra_index, range_bodies_len[1]);
                extra_index += last_body.len;

                if (!first_item) try stream.writeAll(", ");
                first_item = false;
                try self.writeBracedDecl(stream, first_body);
                try stream.writeAll("...");
                try self.writeBracedDecl(stream, last_body);
            }
            if (case.isElse()) {
                assert(first_item); // else prong must not have any items
                try stream.writeAll("else");
            }

            try stream.writeAll(" => ");

            const prong_body = self.code.bodySlice(extra_index, case.prong_info.body_len);
        ...
        try stream.writeAll(") ");
        try self.writeSrcNode(stream, zir_switch.src_node_offset);
    }
